...
- **Eval 基礎設施修復（2026-07-18）**：本次迴歸掃描揪出一個**既有結構性 bug**（與本借鏡改動無關，但被掃描觸發）：使用者將 product-playbook 設為 per-project 啟用（user 層 `enabledPlugins` false ＋ repo 的 `.claude/settings.local.json` true），而 eval 從 `$HOME` 跑 `claude -p`，只有 user 層 false 生效，導致 headless roster **完全沒有 product-playbook**（實測 init 事件 0 條目）——trigger eval 從該 scoping 生效起就整組結構性歸零，與模型或內容無關。已修 `evals/eval_env.py`：`plugin_isolation_args()` 現自帶 `product-playbook@skills-dir: true` 強制啟用受測 plugin（可用 `PRODUCT_PLAYBOOK_EVAL_ENSURE_SELF=0` 關閉），修後 probe 確認 headless roster 恢復 18 個 skills。新增 `tests/test_eval_env.py` 三個回歸測試釘住此行為。
- **乾淨 trigger 數字待補（帳號額度）**：修復後本 session 已無法取得乾淨分數——連續探測撞到帳號層級 Fable 額度上限，`claude -p` 回「You've reached your Fable 5 limit」，`--model sonnet` 亦不例外。0/25 分的掃描輸出屬額度污染的假數據，已刪除不予保留。額度重置後以 `npm run eval:trigger` 重跑即可得真實基線。另記兩個 detection 限制：dev-discipline 的 SessionStart digest hook 在 headless 不發射（既知，見 MEMORY），且 dev-discipline 對 code 輸出依設計不掛 provenance，故其 headless 觸發只能靠正式 Skill call；探索型 meta-skill prompt 在單次 headless（無後續 user turn）會先問澄清問題就停，不產 deliverable 故無 provenance——兩者都使單次 headless 觸發率偏低估，互動模式不受影響（額度未耗盡前的 sonnet 探測確認 meta-skill 仍以 JTBD/prioritization/pre-mortem 角度 inline 運作，無行為迴歸）。`evals/trigger-eval-dev-discipline.json`（8 案例）保留作為 headless 遞送缺口的探針。
