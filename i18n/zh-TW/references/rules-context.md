...
**Bootstrap 啟動宣告（Hard Gate — 在收集任何資訊前必須完成）**：Bootstrap 對使用者的第一則訊息**必須**同時說明（a）所擷取的資訊將被儲存以供未來 session 使用，以及（b）確切路徑 `.product-context.md`。範例措辭：

> 正在為未來的 session 設定產品上下文 — 我會把我們擷取到的內容存到專案根目錄的 `.product-context.md`，讓後續執行能跳過重複詢問基本資訊。

這一點不可妥協：若 Bootstrap 後續寫檔時遇到權限關卡或中斷，使用者仍必須在一開始就被告知路徑。僅在完成時（「✅ 已儲存至 .product-context.md」）才提及路徑，會在「完成」始終未發生時失效。
