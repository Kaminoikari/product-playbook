...
**Bootstrap 시작 안내 (Hard Gate — 정보 수집 전 필수):** Bootstrap의 첫 번째 사용자 대면 메시지는 (a) 수집된 정보가 향후 세션을 위해 저장된다는 점, 그리고 (b) 정확한 경로 `.product-context.md` 두 가지를 반드시 명시해야 합니다. 예시 문구:

> 향후 세션을 위해 제품 컨텍스트를 설정하는 중입니다 — 수집한 내용을 프로젝트 루트의 `.product-context.md`에 저장하여 이후 실행 시 기본 정보를 다시 묻지 않도록 합니다.

이것은 타협 불가능합니다: Bootstrap 파일 쓰기가 나중에 권한 게이트나 중단에 부딪히더라도, 사용자는 경로를 미리 들었어야 합니다. 경로를 완료 시점에만("✅ .product-context.md에 저장됨") 언급하는 것은 완료가 발생하지 않을 때 실패합니다.
