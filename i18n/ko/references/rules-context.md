...
Bootstrap은 Step 0이며, baseline context를 수집하는 것이 목적입니다. **Bootstrap 자체는 정지 지점이 아닙니다**:

- **기본 동작**: Bootstrap과 S1은 **같은 턴** 안에서 S0 → S1 순서로 실행되어야 합니다. 정지 지점은 **S1 완료 후** 사용자 확인을 기다려 S2로 진행하는 지점에 고정되며, S0/S1 사이가 아닙니다.
- **사용자 메시지에 필요한 필드가 이미 제공된 경우**(Section 7 각 모드의 "필수" 필드 기준 참조, 예: 기능 확장 필수 = Identity + Architecture & Tech Stack) → 알려진 필드를 표로 확인만 하고 사용자에게 다시 묻지 않으며, 즉시 S1으로 진행.
- **일부 필드가 누락된 경우** → Bootstrap은 **같은 턴 안에서** 알려진 / 미수집 필드를 표로 제시한 뒤 **즉시 S1으로 진입**합니다. 미확인 필드에는 플레이스홀더를 사용하고, 미수집 필드는 **S1 확인 질문의 일부**로 묶어 사용자가 한 번에 답할 수 있게 합니다.
- **금지 사항**: S0과 S1 사이에 "Round 1 답변을 기다려야 S1으로 진행 가능"이라는 정지 지점을 두는 것. 응답 안에서 S1이 여전히 `⬜ pending`인데 플로우가 사용자 입력 대기로 멈춰 있다면, 본 규칙 위반으로 간주됩니다.

### Bootstrap 완료 후

수집된 정보를 `.product-context.md`에 쓰고 (일부 필드가 플레이스홀더라도 baseline을 먼저 기록; 사용자가 S1 확인 시 보완한 내용으로 덮어쓰기), 같은 턴 안에서 해당 모드의 S1으로 진입하여 S1 내용을 출력합니다.
